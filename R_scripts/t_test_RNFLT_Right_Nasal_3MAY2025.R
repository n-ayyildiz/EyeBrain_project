...
# Step 1: Read CSV files into 'RIGHTRet' with Left and Right Calcarine
Left_Calcarine <- read.csv("Right_N_Beta_4_Left_Calc.csv", header = FALSE, col.names = c("Left_Calcarine"))
Right_Calcarine <- read.csv("Right_N_Beta_4_Right_Calc.csv", header = FALSE, col.names = c("Right_Calcarine"))
...
# Step 5: Perform directed (one-sided) t-test in "greater" direction
t_test <- t.test(Left_Calcarine_clean, Right_Calcarine_clean, 
                 var.equal = variance_test$p.value > 0.05, 
                 alternative = "greater")
t_test

# Step 6: Show the t-test results
cat("\nOne-sided t-test (Left > Right) Results:\n")
...
# Step 9: Perform independent one-sided t-test in "greater" direction
...
# Step 11: Define y-position for annotation manually
annotation_y <- 5.1e-04  # Slightly above the final y-axis tick
...
p <- ggplot(plot_data, aes(x = Group, y = Mean)) +
  ...
  ggtitle("One-sided t-test (Left > Right) Results for Right Nasal") +
  ylab("Mean Beta") +
...
ggsave("independent_t_test_Results for Right Nasal_3MAY2025_right.png", plot = p)
...
library('svglite')
# Step 10: Save the plot
ggsave("independent_t_test_Results for Right Nasal_3MAY2025_right.svg", plot = p)
###
###
