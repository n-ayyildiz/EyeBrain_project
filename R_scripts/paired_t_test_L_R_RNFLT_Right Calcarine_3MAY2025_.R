...
Left_Nasal <- read.csv("Left_N_Beta_4_Right_Calc.csv", header = FALSE, col.names = c("Left_Nasal"))
Right_Nasal <- read.csv("Right_N_Beta_4_Right_Calc.csv", header = FALSE, col.names = c("Right_Nasal"))
...
effect_size <- cohens_d(Left_Nasal_clean, Right_Nasal_clean, paired = TRUE, alternative = "greater")
...
# Step 11: Set manual annotation position and axis limits
annotation_y <- 5.05e-04  # Just above 5e-04
...
p <- ggplot(plot_data, aes(x = Pairs, y = Mean)) +
  ...
  ggtitle("One-sided t-test (Left > Right) Results for Right Calcarine") +
  ylab("Mean Beta") +
...
ggsave("Paired_t_test_Results for Right Calcarine_3MAY2025.png", plot = p)

#install.packages('svglite')
library('svglite')
# Step 10: Save the plot
ggsave("Paired_t_test_Results for Right Calcarine_3MAY2025.svg", plot = p)
